...
import Task as Task
import Util exposing (errorToString, userToString, successStyle, errorStyle)
...
type Msg
    ...
    | CompletedLogin (Result Http.Error GoTrue.User)
    | GotSession Session

-- goTrueToViewer : GoTrue.User -> Viewer
-- goTrueToViewer user =
--     Viewer Nothing (Cred ((Username user.email) user.token))
...
update msg model =
    case msg of
        SubmittedForm ->
            case validate model.form of
                Ok _ ->
                    let
                        config = model.config
                        goTrueUrl = Env.goTrueUrl
                        newConfig = { config| goTrueUrl = goTrueUrl}
                    in
                    
                    ( { model | problems = [] }
                    , Task.attempt CompletedLogin (GoTrue.login newConfig model.form.email model.form.password)
                    )
        ...
        CompletedLogin (Ok user) ->
            let
                viewer = Viewer (Avatar Nothing ) (Cred (Username user.email) user.token.accessToken)
            in
